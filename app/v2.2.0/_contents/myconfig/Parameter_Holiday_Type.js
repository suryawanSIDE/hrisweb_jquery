    ...
    function __Global_Data_This(tagId) {
        ...
        */
		...
		globalData[tagId]['dataEvent']['confirm_delete_Key'] = {
											'nextFunction': 'Parameter_Holiday_Type_Event',
											'action': 'delete'
											}
            ...
            success: (response) => {    
                ...
                if (myObj.status === 'success') {
                    if (getObj.reqAction === 'view') {
                        ...
                        // > modify
						const eventDetail 		= 'Parameter_Holiday_Type_Event(`'+ tagId +'`, `detail_Key`)';
						const eventAdd 			= 'Parameter_Holiday_Type_Event(`'+ tagId +'`, `add_Key`)';
						const eventEdit			= 'Parameter_Holiday_Type_Event(`'+ tagId +'`, `edit_Key`)';
...
// ======== FORM
    ...
    function _Form(getObj) {
        ...
        */
        ...
        const eventSave_All 	= 'Parameter_Holiday_Type_Event(`'+ tagId +'`, `save_Key`)';
        const eventNewForm  	= 'Parameter_Holiday_Type_Event(`'+ tagId +'`, `add_Key`)';
		const eventReload_All	= 'Parameter_Holiday_Type_Event(`'+ tagId +'`, `reload_Key`)';
		set_Form_Button({
            'tagId': tagId,
            'action': action,
            ...
            switch(action) {
				...
				case 'reload':
					...
					const mytimer = setTimeout(() => {						
						...
						Parameter_Holiday_Type_Event(tagId, 'edit_Key')
					}, 5); // 5 ms
...
//> modify module
function Parameter_Holiday_Type_Event(tagId, eventId, targetThis) {
