...
function List_Autofill_City(tagId, colId, searchForm) {
	...
	if (statusEl === false) {
		if (statusInnerEl === '') {
			const result = '<div class="select-content">'+
								...
								'<table><tr>'+
									'<td><input onkeypress="_press_Input_List_City(event, `'+ tagId +'`, '+ dataTable_Index +', `'+ colId +'`)" type="text" class="input-serach-list form-control input-sm" placeholder="search" /></td>'+
									'<td><button onclick="Search_List_Autofill_City(`'+ tagId +'`, `'+ colId +'`)" class="btn btn-default btn-sm autofill-action-search" ><span class="glyphicon glyphicon-search"></span></button></td>'+
									'<td><button onclick="Reset_List_Autofill_City(`'+ tagId +'`, `'+ colId +'`)" class="btn btn-default btn-sm autofill-action-refresh" ><span class="glyphicon glyphicon-refresh"></span></button></td>'+
								'</tr></table>'+
								'</div>'+
			...
			_show_List_Autofill_City(tagId, colId); 
		}
	} else {
		_hide_List_Autofill_City(tagId, colId);
	}
}


function _press_Input_List_City(e, tagId, dataTable_Index, colId) {
...
function Search_List_Autofill_City(tagId, colId) {
...
function Reset_List_Autofill_City(tagId, colId) {
...
function _List_Fetch_City(getObj) {
	...
	const parent_code		= globalData[tagId]['dataForm'][form_Index]['col_code_province'];
...
function _select_List_Autofill_City(eventParam, tagId, colId, indexData, targetThis) {
...
function _show_List_Autofill_City(tagId, colId) {
...
function _hide_List_Autofill_City(tagId, colId) {
